...
namespace VinylStore.DAL.Tests.ArtistTests
{
	...
	{
		[TestMethod]
		public void NullArtistInserted_ThrowException()
		{
			...
			using (var context = new VinylStoreDbContext(options))
			{
				...
				Assert.ThrowsException<ArgumentNullException>(() => artistRepository.Insert(null));
			}

		}

		[TestMethod]
		public void CorrectArtistInserted()
		{
			var options = new DbContextOptionsBuilder<VinylStoreDbContext>()
				.UseInMemoryDatabase(databaseName: MethodBase.GetCurrentMethod().Name)
				.Options;
			...
			using (var context = new VinylStoreDbContext(options))
			{
				...
				var result = artistRepository.Insert(artist);
				context.SaveChanges();
